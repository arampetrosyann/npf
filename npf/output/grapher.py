...
#                    graph.title = build._pretty_name + " - " + graph.title
    ...
    def plot_graphs(self, graphs, filename, fileprefix, f_series=None):
        """
        Render graphs using the configured backend.

        ``graph_backend=default`` (default): classic NPF matplotlib layout.
        ``graph_backend=lux``: Lux auto-visualization (uses ``f_series`` when
        provided so pre-aggregation / pre-extraction variables remain columns).
        """
        backend = str(self.config("graph_backend", "default") or "default").strip().lower()
        if backend == "lux":
            return plot_graphs_with_lux(self, graphs, filename, fileprefix, f_series=f_series)
        if backend != "default":
            print(f"WARNING: Unknown graph_backend={backend!r}, using default")
